...
import json from './assets/data/accounts.json'
...
export function bootstrap(store) {
  ...
  const datafile = process.env.DATAFILE || 'public/data/accounts.json'
  ...
  app2.post('/identity/v2/oauth2/token', (req, res) => {
    ...
    if (store.get('client_id') !== '' && store.get('client_secret') !== '') {
      if (clientId !== store.get('client_id') || clientSecret !== store.get('client_secret')) {
        return res.status(401).json()
      }
    }
    // Generate a token string
    const token = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)
    const expiryDate = dayjs().add(store.get('token_expiry'), 'seconds').format()
    accessTokens[token] = { expires_at: expiryDate, scope: 'accounts' }
    return res.json({ access_token: token, token_type: 'Bearer', expires_in: store.get('token_expiry'), scope: 'accounts' })
  })
  ...
  app2.get('/za/pb/v1/accounts', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
  ...
  app2.get('/za/pb/v1/accounts/:accountId/balance', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
  ...
  app2.get('/za/pb/v1/accounts/:accountId/transactions', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    const accountId = req.params.accountId
  ...
  app2.get('/za/v1/cards/:cardkey/code', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
  ...
  app2.get('/za/v1/cards/:cardkey/publishedcode', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    const cardkey = req.params.cardkey
  ...
  app2.post('/za/v1/cards/:cardkey/environmentvariables', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    const cardkey = req.params.cardkey
  ...
  app2.get('/za/v1/cards/:cardkey/environmentvariables', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    const cardkey = req.params.cardkey
  ...
  app2.get('/za/v1/cards/countries', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    res.json(countries)
  ...
  app2.get('/za/v1/cards/currencies', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    res.json(currencies)
  ...
  app2.get('/za/v1/cards/merchants', (req, res) => {
    if (!isValidToken(req)) {
      return res.status(401).json()
    }
    res.json(merchants)
  ...
  app2.listen(port, () => console.log(`Programmable banking sim listening on port ${port}!`))
  return app2
}